...
export function serviceEntry(slug: string): SeoEntry | undefined {
  ...
  return {
    ...
    keywords: s.metaKeywords ?? [s.keyword, ...keywordsFrom(s.metaDescription)].filter(Boolean),
...
export function blogEntry(slug: string): SeoEntry | undefined {
  ...
  return {
    ...
    keywords:
      p.metaKeywords ??
      ([p.primaryKeyword, ...(p.secondaryKeywords ?? []), p.category, ...keywordsFrom(`${p.title} ${p.excerpt}`)].filter(Boolean) as string[]),
    ogImage: p.image,
...
export function caseStudyEntry(slug: string): SeoEntry | undefined {
  ...
  return {
    ...
    keywords: c.metaKeywords ?? [c.industry, c.service, c.country, "case study", "Webamazee"],
    ogImage: c.image,
...
export function portfolioEntry(slug: string): SeoEntry | undefined {
  ...
  return {
    ...
    path: `/work/${p.slug}`,
    keywords: p.metaKeywords ?? [p.category, p.industry, p.country, "portfolio", "web design", "Webamazee"],
    ogImage: p.image,
    twitterImage: p.image,
    schemaType: "article",
    slug: p.slug,
